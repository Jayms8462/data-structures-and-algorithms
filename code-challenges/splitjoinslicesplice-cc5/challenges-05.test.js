...
const howMuchPencil = (str) => {
  ...
  for (var i = 0; i < str.length + 1; i++){
 	  let test = str.slice(i, str.length);
  	result.push(test);
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((ingredient) => {
    let removeFirst = ingredient.slice(ingredient.indexOf(' ')+1);
    let removeSecond = removeFirst.slice(removeFirst.indexOf(' ')+1);
    result.push(removeSecond);
  });
  return result;
};
