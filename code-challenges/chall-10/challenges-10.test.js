...
const count = (target, input) => {
    let output = 0;
    input.map(test => {
        test.map(test2 => { test2 === target ? output++ : test2 })
    })
    return output;
};
...
const divisibleByFiveTwoToThePower = (input) => {
    let test = input.map(test => {
        let output = [];
        test.map(test2 => {
            if (Number.isInteger(test2)) {
                test2 % 5 === 0 ? output.push(Math.pow(2, test2)) : test2;
            }
        })
        return output;
    });
...
let findMaleAndFemale = (data) => {
    ...
    data.reduce((acc, index) => {
        if (index.gender === 'female' || index.gender === 'male') {
            test.push(index.name);
        }
    }, 0);
    return test.join(' and ');
};
