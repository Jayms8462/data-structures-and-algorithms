...
const countNumberOfElements = (arr) => {
  let sum = arr.reduce(function(index){
    let inx = index + 1
    return inx;
  });
  return sum;
...
const reversedString = (str) => {
  ...
  let test = str.length;
  let work = str.split('');
  work.reduce((acc, index) => {
    test = test - 1;
    strArr.push(work[test]);
  }, 0);
  strArr = strArr.join('');
  return strArr;
};
...
const countNumberOfChildren = (arr) => {
  let test = arr.filter(childCount => childCount.children); //filters out objects without children property
  let test2 = test.map((val, index) => { //returns length of each child property
    return val.children.length;
  }, 0);
  return test2.reduce((acc, value, index) => { //adds all the elements from map
    acc = acc + value;
    return acc;
  }, 0);
...
const calculateAverage = (arr) => {
  let test = arr.reduce((acc, val, inx) => {
    acc = acc + val;
    return acc;
  }, 0)
