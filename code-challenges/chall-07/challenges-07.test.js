...
const forLoopTwoToThe = (arr) => {
  let outArr = [];
  arr.map(num => {
    let pushToArr = Math.pow(2, num);
    outArr.push(pushToArr);
  })
  return outArr;
};
...
const extractAbilities = (arr) => {
  let abname = arr.map((value)=>{return value.ability.name});
  return abname;
};
...
const extractStats = (arr) => {
  let stats = arr.map(test => {
    return {name: test.stat.name, total: (test.effort + test.baseStat),};
  });
  return stats;
};
